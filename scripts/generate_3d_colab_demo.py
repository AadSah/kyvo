...
class InferenceRecipe:
    ...
    def plot_groundtruth_and_prediction(self, gt_dict, pred_dict):
        # Map shape strings to matplotlib markers
        marker_map = {"sphere": "o", "cube": "s", "cylinder": "^"}

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))

        # --- Ground Truth (left) ---
        for obj in gt_dict["objects"]:
            x, y, z = obj["3d_coords"]

            # Determine marker size: large => 200, small => 80
            dot_size = 200 if obj["size"] == "large" else 80

            # Marker shape based on 'shape'
            marker_shape = marker_map.get(obj["shape"], "o")  # default 'o' if not found

            # If rubber, make partially transparent to show "texture"
            alpha_val = 0.5 if obj["material"] == "rubber" else 1.0

            # Create a legend label with all attributes
            label = (
                f"{obj['color']} | {obj['material']} | "
                f"{obj['shape']} | {obj['size']}"
            )
            ...
            ax1.scatter(
                ...
                s=dot_size,
                marker=marker_shape,
                color=obj["color"],  # Use the dictionary color
                alpha=alpha_val,
                label=label,
            )
        ...
        ax1.set_xlim([-4, 4])
        ax1.set_ylim([-4, 4])
        ax1.set_aspect("equal", "box")  # keep the plot square

        # Collect unique labels to avoid repetition
        ...
        # --- Prediction (right) ---
        for obj in pred_dict["objects"]:
            x, y, z = obj["3d_coords"]

            dot_size = 200 if obj["size"] == "large" else 80
            marker_shape = marker_map.get(obj["shape"], "o")
            alpha_val = 0.5 if obj["material"] == "rubber" else 1.0
        ...
        plt.tight_layout()
        plt.savefig(f"./colab-demo-groundtruth-prediction.png")
    ...
    def generate(self, cfg: DictConfig):
        ...
        # select a random sample from the text data
        sample_id = torch.randint(0, len(self.text_data_keys), (1,)).item()
        ...
        if cfg.load_image_target_data:
            ...
            if cfg.reorder_image_tokens:
                ...
                image_target_sample = self.reorder_list_optimized(image_target_sample)
        ...
        if cfg.load_text_target_data:
            text_target_sample = self.text_target_tokens[self.text_data_keys[sample_id]]
            ...
            predicted_answers.append(
                {
                    ...
                    "answer": self._tokenizer.decode(only_generated_tokens.tolist()[0]),
                    "key": self.text_data_keys[sample_id],
                }
            )
        ...
        original_image_path = f"./3d-mllm-datasets-and-codebooks/images-and-scenes-for-evaluation/clevr/original_images/images/{self.text_data_keys[sample_id]}"
        # load the png image and show it
        if cfg.dataset_name == "CLEVR":
            img = mpimg.imread(original_image_path)
            imgplot = plt.imshow(img)
            plt.axis("off")
            # save the image
            plt.savefig(f"./colab-demo-sample-image.png")

        self.plot_groundtruth_and_prediction(groundtruth_scenes[0], predicted_scenes[0])
